...
def _hero(c,fs=46,color='#fff',bg='transparent',pad='0 48px',align='flex-start',ta='left'):
    ...
    return (f'<div style="height:170px;display:flex;flex-direction:column;justify-content:center;padding:{pad};background:{bg};flex-shrink:0">'
            f'<div style="font-size:{afs}px;font-weight:900;color:{color};font-family:Georgia,serif;line-height:1.0;margin-bottom:10px;word-break:break-word;text-align:{ta}">{_e(c["title"])}</div>'
...
def _t01(c):
    ...
    body=(
        ...
        f'<div style="font-size:{_title_fs(c["title"],52)}px;font-weight:900;color:{_N};font-family:Georgia,serif;line-height:1.0;word-break:break-word">{_e(c["title"])}</div>'
...
def _t03(c):
    ...
    for i,hl in enumerate(h):
        ...
        rows+=(f'<div style="flex:1;display:flex;align-items:center;padding:8px 52px;background:{bg};border-bottom:1px solid #e8eef4;min-height:0;gap:24px">'
               ...
               f'<div style="min-width:0;flex:1">'
               f'<div style="font-size:19px;font-weight:700;color:{_N};line-height:1.3;margin-bottom:6px">{_e(hl["heading"])}</div>'
               f'<div style="font-size:15px;color:#4a6080;line-height:1.55;overflow-wrap:break-word">{_e(hl["body"])}</div>'
               f'</div></div>')
    body=(
        ...
        f'<div style="font-size:{_title_fs(c["title"],58)}px;font-weight:900;color:{_N};font-family:Georgia,serif;line-height:1.0;word-break:break-word">{_e(c["title"])}</div>'
...
def _t04(c):
    ...
    main=(f'<div style="flex:1;display:flex;flex-direction:column;background:#f8fafc;min-width:0">'
          ...
          f'<div style="font-size:{_title_fs(c["title"],40,22)}px;font-weight:900;color:{_N};font-family:Georgia,serif;line-height:1.05;word-break:break-word">{_e(c["title"])}</div>'
...
def _t06(c):
    ...
    for i,hl in enumerate(h):
        ...
        rows+=(f'<div style="flex:1;display:flex;align-items:center;padding:8px 52px;background:{bg};border-bottom:1px solid #d8ece0;min-height:0;gap:16px">'
               ...
               f'<div style="min-width:0;flex:1"><div style="font-size:18px;font-weight:700;color:#064e3b;font-family:Georgia,serif;margin-bottom:5px">{_e(hl["heading"])}</div>'
               f'<div style="font-size:14px;color:#3d6b55;line-height:1.55;overflow-wrap:break-word">{_e(hl["body"])}</div></div></div>')
    body=(
        ...
        f'<div style="font-size:{_title_fs(c["title"],44,22)}px;font-weight:900;color:#064e3b;font-family:Georgia,serif;line-height:1.0;word-break:break-word">{_e(c["title"])}</div>'
...
def _t07(c):
    ...
    body=(
        ...
        f'<div style="font-size:{_title_fs(c["title"],46,22)}px;font-weight:900;color:#fff;font-family:Georgia,serif;line-height:1.0;word-break:break-word">{_e(c["title"])}</div>'
...
def _t08(c):
    ...
    body=(
        ...
        f'<div style="font-size:{_title_fs(c["title"],62)}px;font-weight:900;color:#fff;font-family:Georgia,serif;line-height:0.95;word-break:break-word">{_e(c["title"])}</div>'
...
def extract_content(raw: str) -> dict:
    ...
    system = (
        ...
        '  "highlights": [\n'
        '    {"heading": "Topic-specific feature or point", "body": "2 sentences explaining it based on the input content."},\n'
        '    ... (6 total)\n'
        '  ],\n'
    ...
    if not isinstance(d.get('highlights'), list) or len(d['highlights']) < 6:
        d['highlights'] = [
            {'heading': f'{topic_title} Overview',    'body': f'{topic_title} is enabling organizations to automate complex workflows and reduce operational overhead. It delivers measurable efficiency gains across departments.'},
            {'heading': 'Intelligent Automation',     'body': 'AI-driven automation eliminates repetitive manual tasks across enterprise functions. Teams focus on high-value strategic work instead of routine processing.'},
            {'heading': 'Seamless Integration',       'body': 'Modern platforms integrate with existing enterprise systems without disruption. Deployment is fast, with minimal IT overhead and risk.'},
            {'heading': 'Measurable ROI',             'body': 'Organizations achieve significant cost savings and productivity improvements. Results are trackable from day one with built-in analytics.'},
            {'heading': 'Enterprise Scalability',     'body': 'Solutions scale effortlessly from departmental pilots to enterprise-wide rollouts. Performance remains consistent regardless of transaction volume.'},
            {'heading': 'Continuous Improvement',     'body': 'Automated processes learn and improve over time through feedback loops. Organizations stay competitive as technology and needs evolve.'},
        ]
    fallback_hl = [
        {'heading': 'Strategic Value',        'body': f'{topic_title} delivers strategic value across the enterprise. Organizations gain competitive advantage through intelligent automation.'},
        {'heading': 'Process Excellence',     'body': 'Streamlined processes reduce errors and improve consistency. Quality improves across every business function.'},
        {'heading': 'Fast Deployment',        'body': 'Solutions go live quickly with proven implementation frameworks. Minimal disruption to existing operations during rollout.'},
        {'heading': 'Data-Driven Decisions',  'body': 'Real-time insights empower leaders to act with confidence. Analytics surface what matters most for business outcomes.'},
        {'heading': 'Team Empowerment',       'body': 'Employees focus on meaningful work while automation handles repetitive tasks. Productivity and morale improve significantly.'},
        {'heading': 'Future-Ready Platform',  'body': 'Built to adapt as technology and business needs evolve. Organizations remain agile and competitive long-term.'},
    ]

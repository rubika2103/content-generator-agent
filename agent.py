...
def extract_content(raw: str) -> dict:
    ...
    system = (
        ...
        '  "quote_by": "Use a real name/role from the input, or write the source like: Key Insight from [Topic]",\n'
    ...
    raw_json = groq_call(
        ...
        messages=[
            ...
            {'role': 'user',   'content': f'Create poster content strictly based on this input:\n\n{raw[:5000]}'}
        ]
    )
    ...
    topic_title = raw[:60].strip().split('\n')[0] if raw else 'Key Insights'
    d.setdefault('title', topic_title[:50])
    d.setdefault('subtitle', f'A focused look at {topic_title} and its impact on modern business operations.')
    d.setdefault('category', 'Product Insight')
    d.setdefault('problem',
        f'Organizations face significant challenges related to {topic_title}. '
        'Existing approaches often fall short in addressing the complexity and scale required. '
        'A structured and intelligent solution is essential to drive meaningful outcomes.')
    d.setdefault('quote', f'Understanding {topic_title} is essential for any organization looking to improve efficiency and outcomes.')
    d.setdefault('quote_by', f'Key Insight — {topic_title}')
    ...
    if not isinstance(d.get('highlights'), list) or len(d['highlights']) < 6:
        ...
        d['highlights'] = [
            {'heading': 'Core Capability',      'body': f'Addresses the fundamental needs of {topic_title}. Designed for enterprise-scale deployment and reliability.'},
            {'heading': 'Intelligent Approach', 'body': 'Uses structured data and AI to process information accurately. Reduces manual effort significantly across teams.'},
            {'heading': 'Seamless Integration', 'body': 'Connects with existing enterprise tools without disruption. Implementation is fast and requires minimal IT overhead.'},
            {'heading': 'Quality Outcomes',     'body': 'Delivers consistent and measurable results aligned with business goals. Teams can track progress with clear metrics.'},
            {'heading': 'Scalable Solution',    'body': 'Grows with organizational needs without performance degradation. Supports teams of all sizes across multiple locations.'},
            {'heading': 'Actionable Insights',  'body': 'Surfaces key data points that drive better decision-making. Leaders gain clear visibility into what matters most.'},
        ]
    fallback_hl = [
        {'heading': 'Core Capability',      'body': f'Directly addresses {topic_title} with precision and reliability. Built for enterprise use at scale.'},
        {'heading': 'Intelligent Approach', 'body': 'Leverages AI to process and act on information effectively. Reduces errors and improves throughput.'},
        {'heading': 'Seamless Integration', 'body': 'Works with existing systems without disruption. Fast to deploy with minimal IT involvement.'},
        {'heading': 'Quality Outcomes',     'body': 'Produces consistent results aligned with business objectives. Measurable impact from day one.'},
        {'heading': 'Scalable Solution',    'body': 'Handles growing demands without compromise. Trusted by teams across industries.'},
        {'heading': 'Actionable Insights',  'body': 'Gives leaders the data they need to make confident decisions. Real-time visibility into key metrics.'},
    ]

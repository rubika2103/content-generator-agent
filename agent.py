...
STYLE_NAMES = [
    ...
    "Recruitment Card","Automation Roadmap"
]
...
def _title_fs(title, base=52, min_fs=22):
    """Reduce font size for long titles so they fit within fixed-height boxes."""
    n = len(str(title))
    if n <= 30: return base
    if n <= 50: return max(min_fs, base - 10)
    if n <= 70: return max(min_fs, base - 20)
    if n <= 90: return max(min_fs, base - 28)
    return min_fs
...
def _stats_row(stats, num_color, label_color, divider='rgba(0,0,0,0.08)', bg='transparent'):
    ...
    for i, s in enumerate(stats[:4]):
        ...
        parts.append(
            ...
            f'<div style="font-size:32px;font-weight:800;color:{num_color};font-family:\'Inter\',sans-serif;line-height:1">{_e(s["num"])}</div>'
            f'<div style="font-size:12px;color:{label_color};text-align:center;margin-top:5px;line-height:1.3;padding:0 6px">{_e(s["label"])}</div>'
            f'</div>'
        )
    return ''.join(parts)
...
def _html_wrap(body: str) -> str:
    return (
        ...
        'body{width:1024px;height:1536px;overflow:hidden;font-family:"Inter","Liberation Sans","DejaVu Sans",Arial,sans-serif}'
...
def _hl_card_grid(highlights, cols, bg_card, border, accents, head_color, body_color, section_bg='transparent', label_color='#0096b4'):
    ...
    for i, hl in enumerate(highlights[:6]):
        ...
        cards += (
            ...
            f'color:{ac};opacity:0.12;font-family:'Inter',sans-serif;line-height:1">0{i+1}</div>'
            f'<div style="font-size:14px;font-weight:700;color:{head_color};line-height:1.35;margin-bottom:8px">{_e(hl["heading"])}</div>'
            f'<div style="font-size:13px;color:{body_color};line-height:1.65;overflow-wrap:break-word;flex:1">{_e(hl["body"])}</div>'
            f'</div>'
        )
    return (
...
def _hl_full_blocks(highlights, bg_odd, bg_even, accent, head_color, body_color, label_color='#0096b4'):
    ...
    for i, hl in enumerate(highlights[:6]):
        ...
        items += (
            ...
            f'<div style="font-size:38px;font-weight:900;color:{accent};font-family:'Inter',sans-serif;'
            ...
            f'<div style="font-size:14px;color:{body_color};line-height:1.65;overflow-wrap:break-word">{_e(hl["body"])}</div>'
            f'</div></div>'
        )
    return (
...
def _hl_two_col(highlights, border, head_color, body_color, accent, bg='transparent', label_color='#0096b4'):
    ...
    def item(hl, num):
        return (
            ...
            f'<div style="font-size:14px;font-weight:700;color:{head_color};line-height:1.3;margin-bottom:4px">{_e(hl["heading"])}</div>'
            f'<div style="font-size:13px;color:{body_color};line-height:1.6;overflow-wrap:break-word">{_e(hl["body"])}</div>'
            f'</div></div></div>'
        )
...
def _stat_block(stats, num_color, label_color, sep='rgba(255,255,255,0.15)', bg='transparent'):
    ...
    for i, s in enumerate(stats[:4]):
        ...
        parts.append(
            f'<div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;{br}background:{bg}">'
            f'<div style="font-size:30px;font-weight:900;color:{num_color};font-family:'Inter',sans-serif;line-height:1">{_e(s["num"])}</div>'
            f'<div style="font-size:12px;color:{label_color};text-align:center;margin-top:5px;padding:0 8px;line-height:1.4">{_e(s["label"])}</div>'
            f'</div>'
        )
    return ''.join(parts)
...
def _quote_block(quote, quote_by, bg, text_color, attr_color, border_color=None):
    ...
    return (
        ...
        f'<div style="font-size:15px;font-style:italic;color:{text_color};line-height:1.65;margin-bottom:6px;word-break:break-word">"{_e(quote)}"</div>'
...
def _hl_mag_list(highlights, bg='#f8fafc', num_color='#0096b4', head_color='#0d2a3d', body_color='#4a6080', border='#dde8f0'):
    ...
    for i, hl in enumerate(highlights[:6]):
        rows += (
            ...
            f'font-size:36px;font-weight:900;color:{num_color};font-family:'Inter',sans-serif;opacity:0.25;line-height:1;flex-shrink:0">{i+1:02d}</div>'
    ...
    def row(items_slice, bg, row_border_b):
        ...
        for j, hl in enumerate(items_slice):
            ...
            cells += (
                ...
                f'<div style="font-size:14px;font-weight:700;color:{head_color};line-height:1.3;margin-bottom:5px">{_e(hl["heading"])}</div>'
                f'<div style="font-size:13px;color:{body_color};line-height:1.6;overflow-wrap:break-word">{_e(hl["body"])}</div>'
                f'</div>'
            )
...
def _hl_bignum_blocks(highlights, bg_odd, bg_even, accent, head_color, body_color):
    ...
    for i, hl in enumerate(highlights[:6]):
        ...
        items += (
            ...
            f'font-family:'Inter',sans-serif;opacity:0.07;line-height:1;user-select:none">{i+1:02d}</div>'
...
def _hl_compact_dot(highlights, border, head_color, body_color, accent, bg='transparent', label_color='#0096b4'):
    ...
    for hl in highlights[:6]:
        items += (
            ...
            f'<div style="min-width:0;flex:1">'
            f'<div style="font-size:14px;font-weight:700;color:{head_color};line-height:1.25;margin-bottom:3px">{_e(hl["heading"])}</div>'
            f'<div style="font-size:13px;color:{body_color};line-height:1.6;overflow-wrap:break-word">{_e(hl["body"])}</div>'
            f'</div></div>'
        )
    return (
...
def _hl_table_rows(highlights, num_color, head_color, body_color, border, bg_odd, bg_even):
    ...
    for i, hl in enumerate(highlights[:6]):
        ...
        rows += (
            ...
            f'font-size:20px;font-weight:900;color:{num_color};font-family:'Inter',sans-serif;border-right:2px solid {border};flex-shrink:0">{i+1:02d}</div>'
            f'<div style="flex:1;padding:10px 18px;display:flex;flex-direction:column;justify-content:center;min-width:0">'
            f'<div style="font-size:14px;font-weight:700;color:{head_color};line-height:1.3;margin-bottom:4px">{_e(hl["heading"])}</div>'
            f'<div style="font-size:13.5px;color:{body_color};line-height:1.6;overflow-wrap:break-word">{_e(hl["body"])}</div>'
            f'</div></div>'
        )
    return (
        f'<div style="flex:1;display:flex;flex-direction:column;min-height:0">'
...
def _hl_step_boxes(highlights, bg_card, border, head_color, body_color, section_bg='#f4f9fc'):
    ...
    for i, hl in enumerate(highlights[:6]):
        ...
        cards += (
            ...
            f'color:{ac};font-family:'Inter',sans-serif;opacity:0.18;line-height:1">{i+1:02d}</div>'
...
def _hl_diag_2col(highlights, bg, border, head_color, body_color, accents):
    ...
    def item(hl, num, ac):
        return (
            ...
            f'<span style="font-size:10px;font-weight:900;color:{ac};font-family:'Inter',sans-serif;opacity:0.6">{num:02d}</span>'
...
def _hero(c,fs=46,color='#fff',bg='transparent',pad='0 48px',align='flex-start',ta='left'):
    ...
    return (f'<div style="height:170px;display:flex;flex-direction:column;justify-content:center;padding:{pad};background:{bg};flex-shrink:0">'
            f'<div style="font-size:{afs}px;font-weight:900;color:{color};font-family:'Inter',sans-serif;line-height:1.0;margin-bottom:10px;word-break:break-word;text-align:{ta}">{_e(c["title"])}</div>'
...
def _t00(c):
    ...
    cells=''.join(
        ...
        f'<div style="position:absolute;top:6px;right:10px;font-size:26px;font-weight:900;color:{_AC[i]};opacity:0.14;font-family:'Inter',sans-serif">{i+1:02d}</div>'
        f'<div style="font-size:13px;font-weight:700;color:#fff;margin-bottom:5px;line-height:1.3">{_e(h[i]["heading"])}</div>'
        f'<div style="font-size:13px;color:rgba(255,255,255,0.7);line-height:1.6;overflow-wrap:break-word">{_e(h[i]["body"])}</div>'
    ...
    stat_chips=''.join(
        ...
        f'<div style="font-size:24px;font-weight:900;color:{[_A,_L,_O,_T][i]};font-family:'Inter',sans-serif">{_e(c["stats"][i]["num"])}</div>'
...
def _t01(c):
    ...
    def row(hl,num):
        return(f'<div style="flex:1;display:flex;flex-direction:column;justify-content:center;padding:10px 0;border-bottom:1px solid #e2eaf2;min-height:0">'
               f'<div style="display:flex;gap:10px;align-items:flex-start">'
               f'<div style="font-size:30px;font-weight:900;color:{_T};font-family:'Inter',sans-serif;opacity:0.22;min-width:36px;line-height:1">{num:02d}</div>'
    ...
    body=(
        ...
        f'<div style="font-size:{_title_fs(c["title"],52)}px;font-weight:900;color:{_N};font-family:'Inter',sans-serif;line-height:1.0;word-break:break-word">{_e(c["title"])}</div>'
        ...
        f'<div style="font-size:40px;color:#e2eaf2;font-family:'Inter',sans-serif;margin-right:16px;line-height:1;flex-shrink:0">"</div>'
...
def _t02(c):
    ...
    cards=''.join(
        ...
        f'<div style="font-size:14px;font-weight:700;color:{_N};margin-bottom:6px;line-height:1.3">{_e(h[i]["heading"])}</div>'
...
def _t03(c):
    ...
    for i,hl in enumerate(h):
        ...
        num_col=_AC[i]
        rows+=(f'<div style="flex:1;display:flex;align-items:center;padding:8px 52px;background:{bg};border-bottom:1px solid #e8eef4;min-height:0;gap:24px">'
               f'<div style="font-size:48px;font-weight:900;color:{num_col};font-family:'Inter',sans-serif;opacity:0.2;min-width:56px;text-align:right;flex-shrink:0;line-height:1">{i+1:02d}</div>'
               f'<div style="width:3px;align-self:stretch;background:{num_col};opacity:0.35;flex-shrink:0;margin:10px 0"></div>'
               f'<div style="min-width:0;flex:1">'
               f'<div style="font-size:22px;font-weight:700;color:{_N};line-height:1.3;margin-bottom:7px">{_e(hl["heading"])}</div>'
               f'<div style="font-size:16px;color:#4a6080;line-height:1.6;overflow-wrap:break-word">{_e(hl["body"])}</div>'
               f'</div></div>')
    body=(
        f'<div style="display:flex;flex-direction:column;width:1024px;height:1536px;background:#fff;overflow:hidden">'
        f'<div style="height:10px;background:{_T};flex-shrink:0"></div>'
        +_hdr(c,bg='#fff',badge_bg='#e6f4fb',badge_col=_B,badge_brd=_T)
        +f'<div style="min-height:160px;display:flex;flex-direction:column;justify-content:center;padding:16px 52px;background:#fff;border-bottom:3px solid {_T};flex-shrink:0">'
        f'<div style="font-size:{_title_fs(c["title"],58)}px;font-weight:900;color:{_N};font-family:'Inter',sans-serif;line-height:1.0;word-break:break-word">{_e(c["title"])}</div>'
        f'<div style="font-size:15px;color:#4a6080;margin-top:10px;line-height:1.5">{_e(c["subtitle"])}</div>'
        f'<div style="display:flex;gap:8px;margin-top:10px">{_tags_row(c["tags"],"#e6f4fb",_B,_T)}</div></div>'
...
def _t04(c):
    ...
    right_stats=''.join(
        ...
        f'<div style="font-size:26px;font-weight:900;color:{[_A,_L,_O,_T][i]};font-family:'Inter',sans-serif;line-height:1">{_e(c["stats"][i]["num"])}</div>'
    ...
    main=(f'<div style="flex:1;display:flex;flex-direction:column;background:#f8fafc;min-width:0">'
          ...
          f'<div style="font-size:{_title_fs(c["title"],40,22)}px;font-weight:900;color:{_N};font-family:'Inter',sans-serif;line-height:1.05;word-break:break-word">{_e(c["title"])}</div>'
...
def _t06(c):
    ...
    kpi=''.join(
        ...
        f'<div style="font-size:28px;font-weight:900;color:{["#d97706","#d97706",_A,_L][i]};font-family:'Inter',sans-serif">{_e(c["stats"][i]["num"])}</div>'
    ...
    for i,hl in enumerate(h):
        ...
        rows+=(f'<div style="flex:1;display:flex;align-items:center;padding:8px 52px;background:{bg};border-bottom:1px solid #d8ece0;min-height:0;gap:16px">'
               f'<div style="width:36px;height:36px;min-width:36px;background:#064e3b;border-radius:4px;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:#d97706">{i+1:02d}</div>'
               f'<div style="min-width:0;flex:1"><div style="font-size:20px;font-weight:700;color:#064e3b;font-family:\'Inter\',sans-serif;margin-bottom:6px">{_e(hl["heading"])}</div>'
               f'<div style="font-size:15px;color:#3d6b55;line-height:1.6;overflow-wrap:break-word">{_e(hl["body"])}</div></div></div>')
    body=(
        ...
        f'<div style="min-height:140px;display:flex;flex-direction:column;justify-content:center;padding:16px 52px;background:#fff;flex-shrink:0">'
        f'<div style="font-size:{_title_fs(c["title"],44,22)}px;font-weight:900;color:#064e3b;font-family:'Inter',sans-serif;line-height:1.0;word-break:break-word">{_e(c["title"])}</div>'
...
def _t07(c):
    ...
    for i,((gcol,sz)) in enumerate(sizes_css):
        ...
        tiles+=(f'<div style="{gcol};background:linear-gradient(135deg,rgba(13,42,61,0.95),rgba(10,26,46,0.9));'
                ...
                f'font-weight:900;color:{ac};opacity:0.1;font-family:'Inter',sans-serif;line-height:1">{i+1:02d}</div>'
    ...
    body=(
        ...
        f'<div style="font-size:{_title_fs(c["title"],46,22)}px;font-weight:900;color:#fff;font-family:'Inter',sans-serif;line-height:1.0;word-break:break-word">{_e(c["title"])}</div>'
...
def _t08(c):
    ...
    for i,hl in enumerate(h):
        ...
        items+=(f'<div style="flex:1;display:flex;flex-direction:column;justify-content:center;padding:14px 16px;{border_r}{border_b}overflow:hidden">'
                f'<div style="font-size:26px;font-weight:900;color:{_AC[i]};font-family:'Inter',sans-serif;opacity:0.6;line-height:1;margin-bottom:6px">{i+1:02d}</div>'
                f'<div style="font-size:13px;font-weight:700;color:#fff;margin-bottom:5px;line-height:1.3">{_e(hl["heading"])}</div>'
                f'<div style="font-size:13px;color:rgba(255,255,255,0.7);line-height:1.6;overflow-wrap:break-word">{_e(hl["body"])}</div>'
                f'</div>')
    body=(
        f'<div style="display:flex;flex-direction:column;width:1024px;height:1536px;background:#071524;overflow:hidden">'
        # Diagonal clip hero
        ...
        f'<div style="font-size:{_title_fs(c["title"],62)}px;font-weight:900;color:#fff;font-family:'Inter',sans-serif;line-height:0.95;word-break:break-word">{_e(c["title"])}</div>'
...
def _t10(c):
    ...
    cells=''.join(
        ...
        f'<div style="font-size:22px;font-weight:900;color:{_AC[i]};font-family:'Inter',sans-serif;line-height:1;margin-bottom:6px">{i+1:02d}</div>'
...
def _t11(c):
    """Style 11 — Recruitment Card · Deloitte-style two-col checklist with why-section."""
    h = c['highlights'][:6]

    def chk(heading, body, ac='#0096b4'):
        short = body[:110].rsplit(' ', 1)[0] if len(body) > 110 else body
        return (
            f'<div style="display:flex;align-items:flex-start;gap:9px;margin-bottom:11px">'
            f'<div style="width:20px;height:20px;min-width:20px;border-radius:50%;background:{ac};'
            f'display:flex;align-items:center;justify-content:center;margin-top:2px;flex-shrink:0">'
            f'<div style="width:5px;height:9px;border-right:2px solid #fff;border-bottom:2px solid #fff;'
            f'transform:rotate(45deg);margin-bottom:3px"></div></div>'
            f'<div style="font-size:13px;color:#2d3f50;line-height:1.55;flex:1">'
            f'<strong style="color:#0d2a3d">{_e(heading)}</strong> — {_e(short)}</div></div>'
        )

    left  = ''.join(chk(h[i]['heading'], h[i]['body'], ac=_T) for i in range(3))
    right = ''.join(chk(h[i]['heading'], h[i]['body'], ac=_O) for i in range(3, 6))

    stats3 = ''.join(
        f'<div style="flex:1;display:flex;align-items:center;gap:10px;padding:10px 14px;background:#f4f7fb;'
        f'border:1px solid #dde8f4;border-radius:8px;border-top:3px solid {[_T,_O,_N][i]}">'
        f'<div style="font-size:24px">{["🕐","💼","🏢"][i]}</div>'
        f'<div><div style="font-size:10px;color:#6080a0;text-transform:uppercase;letter-spacing:0.8px;font-weight:600">'
        f'{_e(c["stats"][i]["label"])}</div>'
        f'<div style="font-size:14px;font-weight:700;color:{_N}">{_e(c["stats"][i]["num"])}</div></div></div>'
        for i in range(3)
    )

    why = [('🚀','Scale Fast','Enterprise automation at any volume'),
           ('🏆','Proven ROI','Measurable results from day one'),
           ('⚡','AI-Powered','Intelligent, self-improving automation'),
           ('🤝','Expert Support','Dedicated team for your journey')]
    why_html = ''.join(
        f'<div style="flex:1;display:flex;flex-direction:column;align-items:center;text-align:center;padding:0 4px">'
        f'<div style="width:46px;height:46px;border-radius:50%;background:{[_T,_O,_L,_N][i]};'
        f'display:flex;align-items:center;justify-content:center;font-size:22px;margin-bottom:7px">{ic}</div>'
        f'<div style="font-size:12px;font-weight:700;color:{_N};margin-bottom:3px">{ti}</div>'
        f'<div style="font-size:11px;color:#6080a0;line-height:1.4">{de}</div></div>'
        for i, (ic, ti, de) in enumerate(why)
    )

    body = (
        f'<div style="display:flex;flex-direction:column;width:1024px;height:1536px;background:#fff;overflow:hidden">'
        + f'<div style="height:76px;display:flex;align-items:center;justify-content:space-between;'
          f'padding:0 44px;border-bottom:3px solid {_N};flex-shrink:0">'
          f'{_logo(68)}'
          f'<div style="text-align:right"><div style="font-size:10px;color:#6080a0;letter-spacing:1px">Make an</div>'
          f'<div style="font-size:22px;font-weight:900;color:{_O};font-style:italic;line-height:1">impact</div>'
          f'<div style="font-size:10px;color:#6080a0">that matters</div></div></div>'
        + f'<div style="padding:20px 44px 12px;flex-shrink:0">'
          f'<div style="font-size:{_title_fs(c["title"],54)}px;font-weight:900;color:{_N};'
          f'line-height:1.05;word-break:break-word">{_e(c["title"])}</div></div>'
        + f'<div style="background:{_O};padding:9px 44px;flex-shrink:0">'
          f'<div style="font-size:17px;font-weight:800;color:#fff">{_e(c["category"])}</div></div>'
        + f'<div style="padding:10px 44px;background:#f7fafc;border-bottom:1px solid #dde8f4;flex-shrink:0">'
          f'<div style="font-size:14px;color:#2d3f50;line-height:1.4">📍 {_e(c["subtitle"])}</div></div>'
        + f'<div style="display:flex;gap:10px;padding:12px 44px;border-bottom:1px solid #dde8f4;flex-shrink:0">'
          f'{stats3}</div>'
        + f'<div style="flex:1;display:flex;min-height:0">'
          f'<div style="flex:1;border-right:2px solid #e8eef4;padding:14px 18px 14px 44px;'
          f'display:flex;flex-direction:column;overflow:hidden">'
          f'<div style="background:{_N};color:#fff;font-size:12px;font-weight:800;letter-spacing:1.5px;'
          f'text-transform:uppercase;padding:7px 14px;border-radius:4px;margin-bottom:14px;'
          f'flex-shrink:0;display:inline-block">🔑 KEY HIGHLIGHTS</div>'
          f'{left}</div>'
          f'<div style="flex:1;padding:14px 44px 14px 18px;display:flex;flex-direction:column;overflow:hidden">'
          f'<div style="background:{_T};color:#fff;font-size:12px;font-weight:800;letter-spacing:1.5px;'
          f'text-transform:uppercase;padding:7px 14px;border-radius:4px;margin-bottom:14px;'
          f'flex-shrink:0;display:inline-block">⚡ SKILLS &amp; IMPACT</div>'
          f'{right}</div></div>'
        + f'<div style="padding:16px 44px;background:#f4f7fb;border-top:2px solid #dde8f4;flex-shrink:0">'
          f'<div style="font-size:11px;font-weight:800;color:{_N};letter-spacing:2px;text-transform:uppercase;'
          f'text-align:center;margin-bottom:12px">WHY QUALESCE?</div>'
          f'<div style="display:flex;gap:8px">{why_html}</div></div>'
        + f'<div style="background:{_N};padding:13px 44px;display:flex;align-items:center;'
          f'justify-content:space-between;flex-shrink:0">'
          f'<div style="font-size:12px;color:rgba(255,255,255,0.6)">'
          f'{_e((c.get("tags") or ["Enterprise Automation"])[0])}</div>'
          f'<div style="background:{_O};color:#fff;font-size:13px;font-weight:700;'
          f'padding:9px 22px;border-radius:6px">Explore This Opportunity →</div>'
          f'<div style="font-size:11px;color:rgba(255,255,255,0.4)">www.qualesce.com</div></div>'
        + f'</div>'
    )
    return _html_wrap(body)
...
def _t12(c):
    """Style 12 — Automation Roadmap · numbered steps + 3 pillars + goal section."""
    h = c['highlights'][:6]

    def step_bullets(body, ac):
        sents = [s.strip() for s in body.replace(';', '.').split('.') if len(s.strip()) > 10][:3]
        return ''.join(
            f'<div style="display:flex;align-items:flex-start;gap:6px;margin-bottom:5px">'
            f'<div style="width:6px;height:6px;min-width:6px;border-radius:50%;background:{ac};margin-top:6px"></div>'
            f'<div style="font-size:12px;color:rgba(255,255,255,0.78);line-height:1.45;flex:1">{_e(s)}</div></div>'
            for s in sents
        )
    ...
    for i, hl in enumerate(h):
        ...
        step_cells += (
            ...
            f'border-top:3px solid {ac};border-radius:10px;padding:14px 16px;'
            f'display:flex;flex-direction:column;overflow:hidden">'
            f'<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">'
            f'<div style="width:34px;height:34px;min-width:34px;border-radius:50%;background:{ac};'
            f'display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;'
            f'color:#fff;flex-shrink:0">{i+1:02d}</div>'
            f'<div style="font-size:14px;font-weight:700;color:#fff;line-height:1.25">{_e(hl["heading"])}</div></div>'
            f'{step_bullets(hl["body"], ac)}'
            f'<div style="margin-top:8px">'
            f'<span style="background:rgba(255,255,255,0.1);border:1px solid {ac};border-radius:4px;'
            f'padding:3px 8px;font-size:10px;font-weight:700;color:{ac};letter-spacing:1px;'
            f'text-transform:uppercase">OUTCOME</span></div></div>'
        )

    pillars = ''
    pillar_labels = ['GOVERNANCE', 'SCALABILITY', 'ROI FOCUS']
    pillar_colors = [_T, _O, _L]
    for i in range(3):
        col = pillar_colors[i]
        stat = c['stats'][i]
        pillars += (
            f'<div style="flex:1;background:rgba(255,255,255,0.07);border:1px solid {col};'
            f'border-radius:8px;padding:14px 16px">'
            f'<div style="font-size:11px;font-weight:800;color:{col};letter-spacing:1.5px;'
            f'text-transform:uppercase;margin-bottom:5px">{pillar_labels[i]}</div>'
            f'<div style="font-size:26px;font-weight:900;color:#fff;margin-bottom:4px">{_e(stat["num"])}</div>'
            f'<div style="font-size:12px;color:rgba(255,255,255,0.65);line-height:1.4">{_e(stat["label"])}</div>'
            f'</div>'
        )

    words = str(c['title']).split()
    styled_title = ''.join(
        f'<span style="color:{_O}">{_e(w)} </span>' if i % 3 == 1 else f'{_e(w)} '
        for i, w in enumerate(words)
    )

    body = (
        f'<div style="display:flex;flex-direction:column;width:1024px;height:1536px;'
        f'background:#0a1525;overflow:hidden">'
        + f'<div style="padding:24px 44px 18px;background:#0d1e30;border-bottom:4px solid {_T};flex-shrink:0">'
          f'{_logo(60)}'
          f'<div style="font-size:{_title_fs(c["title"],54,28)}px;font-weight:900;color:#fff;'
          f'line-height:1.05;margin-top:12px;word-break:break-word">{styled_title}</div>'
          f'<div style="margin-top:10px;font-size:14px;color:rgba(255,255,255,0.5)">'
          f'<span style="color:{_T};font-weight:700">A STRATEGIC.</span> '
          f'<span style="color:{_O};font-weight:700">STRUCTURED.</span> '
          f'<span style="color:{_L};font-weight:700">VALUE-DRIVEN</span> approach.</div></div>'
        + f'<div style="flex:1;display:grid;grid-template-columns:1fr 1fr;'
          f'grid-template-rows:repeat(3,1fr);gap:10px;padding:14px 44px;min-height:0">'
          f'{step_cells}</div>'
        + f'<div style="flex-shrink:0;padding:0 44px 14px">'
          f'<div style="font-size:10px;font-weight:800;color:rgba(255,255,255,0.35);letter-spacing:2px;'
          f'text-transform:uppercase;margin-bottom:10px;text-align:center">'
          f'THE 3 PILLARS I NEVER COMPROMISE ON</div>'
          f'<div style="display:flex;gap:10px">{pillars}</div></div>'
        + f'<div style="padding:12px 44px;background:rgba(255,255,255,0.04);'
          f'border-top:1px solid rgba(255,255,255,0.08);display:flex;gap:24px;flex-shrink:0">'
          f'<div style="flex:1">'
          f'<div style="font-size:10px;font-weight:800;color:{_O};letter-spacing:2px;'
          f'text-transform:uppercase;margin-bottom:5px">MY GOAL:</div>'
          f'<div style="font-size:13px;color:rgba(255,255,255,0.72);line-height:1.6;font-style:italic">'
          f'{_e(c["problem"][:160])}</div></div>'
          f'<div style="flex:1;border-left:3px solid {_T};padding-left:16px">'
          f'<div style="font-size:13px;color:rgba(255,255,255,0.72);line-height:1.6;font-style:italic">'
          f'&ldquo;{_e(c["quote"])}&rdquo;</div>'
          f'<div style="font-size:11px;font-weight:700;color:{_T};margin-top:6px">'
          f'— {_e(c["quote_by"])}</div></div></div>'
        + _footer(f'linear-gradient(90deg,#0d1e30,{_B})','#fff','rgba(255,255,255,0.5)')
        + f'</div>'
    )
    return _html_wrap(body)


TEMPLATES = [_t00,_t01,_t02,_t03,_t04,_t05,_t06,_t07,_t08,_t09,_t10,_t11,_t12]

# ── content extraction ────────────────────────────────────────────────────────

def _clean_topic(raw: str) -> str:
    """Strip instruction phrases and extract the actual topic."""
    import re as _re
    clean = raw.strip()
    # Remove common instruction prefixes
    prefixes = [
        r'^create\s+a\s+post\s+about\s+', r'^write\s+a\s+post\s+about\s+',
        r'^generate\s+a\s+post\s+about\s+', r'^make\s+a\s+post\s+about\s+',
        r'^create\s+a\s+blog\s+about\s+', r'^write\s+about\s+',
        r'^post\s+about\s+', r'^blog\s+about\s+', r'^topic\s*:\s*',
        r'^write\s+a\s+blog\s+about\s+', r'^generate\s+content\s+about\s+',
    ]
    for prefix in prefixes:
        clean = _re.sub(prefix, '', clean, flags=_re.IGNORECASE).strip()
    return clean[:200]
...
def extract_content(raw: str) -> dict:
    ...
    system = (
        'You are a professional marketing content writer for Qualesce, an enterprise automation company. '
        'Your job is to extract and write poster content STRICTLY based on the provided input. '
        'Return ONLY valid JSON — no markdown, no explanation, no code blocks.\n\n'
        'CRITICAL RULES:\n'
        '1. NEVER invent company names, people names, or fake statistics not in the input.\n'
        '2. NEVER use placeholder names like "XYZ", "ABC Company", "John Doe", "TechCorp" etc.\n'
        '3. ALL content must be directly derived from the input topic or document.\n'
        '4. If the input mentions specific companies, people, or numbers — use them exactly.\n'
        '5. If no specific stats exist in the input, write descriptive labels without made-up numbers.\n'
        '6. The quote and quote_by must feel authentic to the topic — do not invent random names.\n'
        '7. Stay strictly on topic. Do not add unrelated content.\n\n'
        'Required JSON structure:\n'
        '{\n'
        '  "title": "compelling title based on the actual topic, max 8 words",\n'
        '  "subtitle": "one strong sentence summarizing the core value from the input, 15-20 words",\n'
        '  "category": "one of: Customer Story / Product Insight / Use Case / Industry Trend / How-To / Case Study",\n'
        '  "stats": [\n'
        '    {"num": "metric from input or descriptive word like High/Fast", "label": "specific label from topic"},\n'
        '    {"num": "metric", "label": "label"},\n'
        '    {"num": "metric", "label": "label"},\n'
        '    {"num": "metric", "label": "label"}\n'
        '  ],\n'
        '  "problem": "3 sentences describing the specific challenge from the input. Be precise and topic-relevant.",\n'
        '  "highlights": [\n'
        '    {"heading": "Topic-specific feature or point", "body": "3-4 rich sentences (45-65 words) explaining this point in depth. Cover what it does, why it matters, and the real-world impact. Be specific to the topic — do not write generic filler."},\n'
        '    ... (6 total, each body must be 45-65 words)\n'
        '  ],\n'
        '  "quote": "An insightful statement or key takeaway from the topic, 20-30 words. Do not invent a fake person quote.",\n'
        '  "quote_by": "Short attribution — use a real job title or role from the input (e.g. \'VP of Operations\') if one exists, otherwise write a brief professional label like \'Qualesce Automation Expert\'. NEVER repeat the topic text. NEVER use \'Key Insight\' patterns.",\n'
    ...
    raw_json = groq_call(
        ...
        messages=[
            ...
            {'role': 'user',   'content': f'Create poster content strictly based on this topic:\n\n{topic}\n\nFull content:\n{raw[:5000]}'}
        ]
    )
    ...
    topic_title = topic[:60] if topic else 'Enterprise Automation'
    d.setdefault('title', topic_title[:50])
    d.setdefault('subtitle', f'How {topic_title} is transforming enterprise operations and driving measurable business outcomes.')
    d.setdefault('category', 'Industry Trend')
    d.setdefault('problem',
        f'{topic_title} presents both challenges and opportunities for modern enterprises. '
        'Many organizations struggle to implement it effectively without the right tools and expertise. '
        'A structured approach is critical to unlocking its full potential and avoiding costly missteps.')
    d.setdefault('quote', f'{topic_title} is reshaping how enterprises operate, compete, and deliver value to their customers.')
    d.setdefault('quote_by', 'Qualesce Automation Expert')
    if not isinstance(d.get('stats'), list) or len(d['stats']) < 4:
        d['stats'] = [
            {'num': 'High',   'label': 'Operational efficiency improvement'},
            {'num': 'Fast',   'label': 'Time to value for implementation'},
            {'num': 'Strong', 'label': 'Quality and accuracy outcomes'},
            {'num': 'Proven', 'label': 'Results across enterprise deployments'},
        ]
    if not isinstance(d.get('highlights'), list) or len(d['highlights']) < 6:
        d['highlights'] = [
            {'heading': f'{topic_title} Overview',    'body': f'{topic_title} enables organizations to automate complex workflows and significantly reduce operational overhead. By streamlining repetitive processes, teams can focus on high-value strategic work. This delivers measurable efficiency gains across all departments while improving accuracy and speed of execution.'},
            {'heading': 'Intelligent Automation',     'body': 'AI-driven automation eliminates repetitive manual tasks across enterprise functions, freeing teams to focus on strategic priorities. Intelligent systems adapt to changing inputs and learn from feedback, ensuring consistent output quality. The result is faster throughput, fewer errors, and significantly lower operational costs at scale.'},
            {'heading': 'Seamless Integration',       'body': 'Modern automation platforms integrate with existing enterprise tools — ERP, CRM, ITSM — without requiring costly replacements. Deployment is fast, with pre-built connectors reducing implementation time dramatically. Minimal IT overhead means organizations can go live quickly and start capturing value from day one.'},
            {'heading': 'Measurable ROI',             'body': 'Organizations using this approach report significant cost savings and productivity improvements within the first quarter of deployment. Built-in analytics track every process milestone, giving leadership real-time visibility into performance. Results are quantifiable, auditable, and directly tied to business outcomes that matter.'},
            {'heading': 'Enterprise Scalability',     'body': 'Solutions scale effortlessly from small departmental pilots to enterprise-wide deployments spanning thousands of users. Performance remains consistent regardless of transaction volume or geographic distribution. This elastic capacity means organizations only pay for what they use while retaining the ability to grow without re-platforming.'},
            {'heading': 'Continuous Improvement',     'body': 'Automated processes continuously capture performance data and surface improvement opportunities through machine learning feedback loops. Teams can refine workflows without disruption, adapting to new business requirements in days rather than months. This culture of continuous improvement keeps organizations competitive as market conditions and technology evolve.'},
        ]
    fallback_hl = [
        {'heading': 'Strategic Value',        'body': f'{topic_title} delivers strategic value by automating high-impact processes that previously required significant manual effort. Organizations gain a measurable competitive advantage, with faster cycle times and lower error rates. Leadership gains the visibility needed to make confident, data-driven decisions at every level of the business.'},
        {'heading': 'Process Excellence',     'body': 'Standardized automated workflows eliminate the variability that leads to errors and inefficiency in manual processes. Every step is logged, monitored, and optimized continuously, driving consistent quality across every business function. Teams spend less time on exceptions and more time on work that creates customer and business value.'},
        {'heading': 'Fast Deployment',        'body': 'Proven implementation frameworks reduce time-to-value from months to weeks, with minimal disruption to existing operations. Pre-configured templates and low-code tooling mean business users can configure and launch processes without deep technical expertise. Organizations start capturing ROI earlier while maintaining full control over rollout pace and scope.'},
        {'heading': 'Data-Driven Decisions',  'body': 'Real-time dashboards and process analytics give leaders instant visibility into operational performance across every automated workflow. Exception reports surface bottlenecks before they escalate, enabling proactive management rather than reactive firefighting. Accurate, timely data replaces gut instinct, driving better outcomes at every decision point in the organization.'},
        {'heading': 'Team Empowerment',       'body': 'When automation handles routine and repetitive tasks, employees redirect their energy toward creative problem-solving and customer engagement. Job satisfaction improves as teams work on meaningful, higher-value activities that leverage their expertise. This empowerment drives retention, morale, and organizational agility in a competitive talent market.'},
        {'heading': 'Future-Ready Platform',  'body': 'Built on open standards and cloud-native architecture, the platform adapts as technology and business needs evolve without requiring costly re-implementation. Regular feature updates and an active partner ecosystem ensure organizations always have access to the latest capabilities. Investments made today continue delivering value as the organization scales and transforms.'},
    ]
    while len(d['highlights']) < 6:
        d['highlights'].append(fallback_hl[len(d['highlights'])])
    # Ensure each highlight body has enough content
    for hl in d['highlights']:
        if len(hl.get('body', '').split()) < 25:
            hl['body'] = hl.get('body', '') + ' This approach delivers consistent, measurable results across your entire organization, improving both efficiency and competitive positioning in the market.'
